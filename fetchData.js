  ...
  return axios.request(config)
    .then((response) => {
      ...
      if (response.status === 200) {
        return response.data.data["searchQueries"]["search"]["results"] ?? [];
      }
      return []
    })
    .catch((error) => {
...
async function generateCSV(listings) {
  ...
  const records = listings.map(listing => ({
    id: listing?.basicPropertyData?.id,
    title: listing?.displayName?.text ?? "",
    pageName: listing?.basicPropertyData?.pageName ?? "",
    amountPerStay: listing?.priceDisplayInfoIrene?.priceBeforeDiscount?.displayPrice?.amountPerStay?.amount ?? '-'
  }));
...
async function main() {
  ...
  try {
    const listings = await filterData(address, pageSize);
    console.log(listings);

    if (listings) {
      await generateCSV(listings);
    } else {
